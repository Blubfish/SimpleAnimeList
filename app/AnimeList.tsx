...
export default function AnimeList({ animeList }: AnimeListProps) {
  ...
  const [currentList, setCurrentList] = useState(animeList);
    ...
    <section className="rounded-3xl border border-slate-800/80 bg-linear-to-br from-slate-900/80 to-slate-950/80 p-6 shadow-2xl shadow-black/40 backdrop-blur-xl">
      <div className="mb-5 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-col gap-2 md:items-start">
          <Link
            href="/addAnime"
            className="inline-flex items-center justify-center gap-2 rounded-3xl bg-linear-to-r from-orange-500 to-amber-500 px-4 py-2 text-xs md:text-sm font-bold text-slate-950 shadow-md shadow-orange-500/20 ring-1 ring-orange-300/30 transition hover:brightness-110 hover:shadow-orange-500/40 focus:outline-none focus:ring-2 focus:ring-orange-300/50"
          >
          ...
          <div className="space-y-2">
            ...
            <Combobox
              ...
              onValueChange={async (e) => {
                setListOrder(e ?? "");
                setCurrentList(
                  await getSortAnimeList(e ?? "Score: High to Low", animeList),
                );
              }}
              value={listOrder}
            >
